        ...
        <div class="container_main contents_area">
            <div class="container_box" id="main_box">
                ...
                <div class="review_box">
                    ...
                    <div class="star_box">
                        ...
                        <span id="review_score">${rd.restaurant_total_score_count}</span>
                    </div>

                    <!-- 별점 스코어 밑 후기 갯수 관련 -->
                    <div id="review_leng">후기 ${rd.restaurant_total_review_count}개</div>
                </div>

                <!-- 식당 정보 관련 박스 -->
                <div id="rest_title_box">
                    <p id="rest_title">${rd.restaurant_name}</p>
                    <p id="rest_cate">${rd.restaurant_category}</p>
                    <div class="rest_save">
                ...
                <div id="rest_info_box">
                    <p id="rest_d_addr">
                        ...
                        <c:if test='${rd.restaurant_address == ""}'>
                            매장정보없음
                        </c:if>
                        <c:if test='${rd.restaurant_address != ""}'>
                    ...
                    <p id="rest_p_num">
                        ...
                        <c:if test='${rd.restaurant_number == ""}'>
                            번호정보없음
                        </c:if>
                        <c:if test='${rd.restaurant_number != ""}'>
                            ${rd.restaurant_number}
                        </c:if>
                    </p>
                    <p id="rest_info">
                        <i class="rest_info_icon"></i>
                        ${rd.restaurant_reservation}
                    </p>
                </div>
                <c:if test='${rd.restaurant_memo != ""}'>
                    <p id="rest_intro">${rd.restaurant_memo}</p>
                </c:if>
            </div>
            <!-- 영업 시간 박스 : 타이틀 -->
            <div class="container_box" id="sub_box">
                <div id="rest_time_box">
                    ...
                    <div class="time_box">
                        <p class="time_icon"></p>
                        <p>${rd.today_business_state}</p>
                        <c:choose>
                            <c:when test='${rd.today_business_state == "영업중"}'>
                                <p id="open_time">영업중</p>
                            </c:when>
                            <c:when test='${rd.today_business_state == "영업종료"}'>
                                <p id="close_time">영업종료</p>
                            </c:when>
                            <c:otherwise>
                                <p id="close_time">영업정보 없음</p>
                            </c:otherwise>
                        </c:choose>
<%--                        <p id="break_time">브레이크 타임</p>--%>
                ...
                <!-- 영업 시간 정보 -->
                <div class="day_box">
                    <p>월  ${rd.business_hours_dto.mon}</p>
                    <p>화  ${rd.business_hours_dto.tue}</p>
                    <p>수  ${rd.business_hours_dto.wed}</p>
                    <p>목  ${rd.business_hours_dto.thu}</p>
                    <p>금  ${rd.business_hours_dto.fri}</p>
                    <p>토  ${rd.business_hours_dto.sat}</p>
                    <p>일  ${rd.business_hours_dto.sun}</p>
                    <p>${rd.business_hours_dto.break_time}</p>
                </div>
            </div>

            <div class="container_box" id="menu_container">
                <p class="container_title menu_con_title">${rd.restaurant_name} 메뉴</p>
                <c:forEach var="menu" items="${rd.menu_detail_list}">
                <div class="menu_box">
                    <div class="menu_img"></div>
                    <div class="menu_title_box">
                        <p class="menu_title">${menu.menu_name}</p>
                        <p class="menu_price">${menu.menu_price} 원</p>
                    </div>
                </div>
                </c:forEach>
            </div>
            ...
            <div class="container_box" id="img_container">
                <h1 class="container_title">
                    <c:if test='${fn:length(rd.restaurant_image_url_list) != 0}'>
                        ${rd.restaurant_name + " 이미지 "+fn:length(rd.restaurant_image_url_list)+" 건"}
                    </c:if>
                </h1>
                ...
                <div class="img_con_more_btn">
                    <c:if test='${fn:length(rd.review_image_url_list) != 0}'>
                        <p>사진 더 불러오기</p>
            ...
            <div class="container_box" id="reco_container">
                <div class="container_title">
                    <p>${rd.restaurant_category}</p>
                    <p>추천 맛집</p>
                </div>
                <c:forEach var="tmp" items="${rd.relation_restaurant_list}">
                    <div class="reco_box reco_box1">
                        <div class="reco_title_box">
                            <div class="reco_title">${tmp.restaurant_name}</div>
                            <div class="reco_score">
                                <span id="reco_star" class="reco_empty_star">
                                    <p class="reco_fill_star"></p>
                                </span>
                                <span id="reco_score">${tmp.restaurant_total_score_count}</span>
                                <span>${tmp.restaurant_total_review_count}</span>
                            </div>
                        </div>
                        <div class="reco_addr">${tmp.restaurant_address}</div>
                        <div class="reco_img_box">
                            <div class="reco_img reco_img1">
                                ${tmp.restaurant_image_url_list[0]}
                            </div>
                            <div class="reco_img reco_img2">
                                ${tmp.restaurant_image_url_list[1]}
                            </div>
                        </div>
                    </div>
                </c:forEach>
            ...
            <div class="container_box" id="review_container">
                <div class="rev_con_title">
                    <h1 class="container_title">
                        ${rd.restaurant_name} 방문자 평가 건
                    </h1>
                    <div href="<c:url value="reviewWrite"/>" class="rev_create_btn">후기를 작성해서 맛맵을 지원해주세요.</div>
                </div>
                <!-- 평점 관련단 -->
                    ...
                    <div class="rev_con_score">
                        ...
                        <div class="rev_con_score_star">
                            <div class="rev_con_score_starbox">
                                ...
                                <span class="rev_review_score">4.8</span>
                            </div>
                            <div class="rev_con_score_starbox">
                                <span class="rev_review_star" class="rev_empty_star">
                                    <p class="rev_fill_star" id="star_kind"></p>
                                </span>
                                <span class="rev_review_score">4.5</span>
                            </div>
                            <div class="rev_con_score_starbox">
                                <span class="rev_review_star" class="rev_empty_star">
                                    <p class="rev_fill_star" id="star_clean"></p>
                                </span>
                                <span class="rev_review_score">4.2</span>
                            </div>
                            <div class="rev_con_score_starbox">
                                <span class="rev_review_star" class="rev_empty_star">
                                    <p class="rev_fill_star" id="star_taste"></p>
                                </span>
                                <span class="rev_review_score">4.9</span>
                            </div>
                        </div>
                    </div>
                    <!-- 평점 관련 ( 점수 관련 ) -->
